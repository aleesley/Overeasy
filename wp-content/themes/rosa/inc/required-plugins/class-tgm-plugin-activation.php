...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->domain = 'rosa_txtd';

			self::$instance =& $this;

			$this->strings = array(
				'page_title'                         => __( 'Install Required Plugins', 'rosa_txtd' ),
				'menu_title'                         => __( 'Install Plugins', 'rosa_txtd' ),
				'installing'                         => __( 'Installing Plugin: %s', 'rosa_txtd' ),
				'oops'                               => __( 'Something went wrong.', 'rosa_txtd' ),
				'notice_can_install_required'        => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'rosa_txtd' ),
				'notice_can_install_recommended'     => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'rosa_txtd' ),
				'notice_can_install_update'          => _n_noop( 'There is a new update for %1$s.', 'There are several plugins updates available %1$s.', 'rosa_txtd' ),
				'notice_can_install_required_update' => _n_noop( 'This theme requires the following plugin to be updated: %1$s.', 'This theme requires the following plugins to be updated: %1$s.', 'rosa_txtd' ),
				'notice_cannot_install'              => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'rosa_txtd' ),
				'notice_can_activate_required'       => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'rosa_txtd' ),
				'notice_can_activate_recommended'    => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'rosa_txtd' ),
				'notice_cannot_activate'             => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'rosa_txtd' ),
				'notice_ask_to_update'               => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'rosa_txtd' ),
				'notice_cannot_update'               => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'rosa_txtd' ),
				'install_link'                       => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'rosa_txtd' ),
				'update_link'                        => _n_noop( 'Begin updating plugin', 'Begin updating plugins', 'rosa_txtd' ),
				'activate_link'                      => _n_noop( 'Activate installed plugin', 'Activate installed plugins', 'rosa_txtd' ),
				'return'                             => __( 'Return to Required Plugins Installer', 'rosa_txtd' ),
				'plugin_activated'                   => __( 'Plugin activated successfully.', 'rosa_txtd' ),
				'complete'                           => __( 'All plugins installed and activated successfully. %1$s', 'rosa_txtd' ),
			);
			...
			if ( isset( $_GET[ sanitize_key( 'plugin' ) ] ) && ( ( isset( $_GET[ sanitize_key( 'tgmpa-install' ) ] ) && 'install-plugin' == $_GET[ sanitize_key( 'tgmpa-install' ) ] ) || ( isset( $_GET[ sanitize_key( 'tgmpa-update' ) ] ) && 'update-plugin' == $_GET[ sanitize_key( 'tgmpa-update' ) ] ) ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'rosa_txtd' ) . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' . sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'rosa_txtd' ) . '">' . __( 'Return to the Dashboard', 'rosa_txtd' ) . '</a>' ) . '</p>';
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[ sanitize_key( 'action' ) ] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'rosa_txtd' ), '<strong>' . $plugin['name'] . '</strong>' );
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'rosa_txtd' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */

					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'rosa_txtd' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'rosa_txtd' ) . '</a>' : '';
					$show_update_link   = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'rosa_txtd' ) . '</a>' : '';
					...
					$action_links = apply_filters( 'tgmpa_notice_action_links', array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'rosa_txtd' ) . '</a>',
						) );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[ $i ]['source'] = __( 'External Link', 'rosa_txtd' );
				} elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
						$table_data[ $i ]['source'] = __( 'Private Repository', 'rosa_txtd' );
					} /** The plugin is pre-packaged with the theme */ else {
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'rosa_txtd' );
					}
				} /** The plugin is from the WordPress repository */ else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'rosa_txtd' );
				}

				$table_data[ $i ]['type'] = $plugin['required'] ? __( 'Required', 'rosa_txtd' ) : __( 'Recommended', 'rosa_txtd' );

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'rosa_txtd' ) );
				} elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'rosa_txtd' ) );
				} elseif ( version_compare( $installed_plugins[ $plugin['file_path'] ]['Version'], $plugin['version'], '<' ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Updated', 'rosa_txtd' ) );
				} else {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Active', 'rosa_txtd' ) );
				}
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'rosa_txtd' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'rosa_txtd' ),
				'source' => __( 'Source', 'rosa_txtd' ),
				'type'   => __( 'Type', 'rosa_txtd' ),
				'status' => __( 'Status', 'rosa_txtd' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'rosa_txtd' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'rosa_txtd' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'rosa_txtd' ), $imploded );
				}
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'rosa_txtd' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'rosa_txtd' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'rosa_txtd' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'rosa_txtd' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'rosa_txtd' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'rosa_txtd' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'rosa_txtd' );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'rosa_txtd' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'rosa_txtd' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'rosa_txtd' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'rosa_txtd' ) . '</span><span class="hidden">' . __( 'Hide Details', 'rosa_txtd' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'rosa_txtd' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'rosa_txtd' );
				} /** Default installation strings */ else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'rosa_txtd' );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'rosa_txtd' );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'rosa_txtd' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'rosa_txtd' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'rosa_txtd' ) . '</span><span class="hidden">' . __( 'Hide Details', 'rosa_txtd' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'rosa_txtd' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'rosa_txtd' );
				}
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' . sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'rosa_txtd' ) . '">' . __( 'Return to the Dashboard', 'rosa_txtd' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
